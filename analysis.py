...
class Analysis():
	...
	def __init__(self, feature_path, partic_path, network_labels_path):
		...
		self.test_species = pd.read_csv(os.getcwd() + '/test_species.csv', encoding="ISO-8859-1")
	...
	def make_image_dict(self):
		...
		# Get list of novel species for each dataset
		imagenet = np.array(self.test_species['in imagenet'])
		cub = np.array(self.test_species['in cub'])
		species = np.array(self.test_species['name'])
